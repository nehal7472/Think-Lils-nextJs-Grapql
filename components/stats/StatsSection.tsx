    ...
    <section className="relative bg-[#0A1019] py-20 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-10 text-white relative z-10">
        {stats.map((stat) => (
          <div
            key={stat.id}
            className="flex flex-col items-center bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-10 shadow-lg hover:shadow-emerald-400/20 transition-all text-center"
          >
            {/* Icon container */}
            <div className="flex items-center justify-center w-16 h-16 rounded-full bg-emerald-500/10 mb-6">
              {stat.icon}
            </div>
